...
namespace Wasmtime
{
        ...
        : WasmtimeException
    {
        ...
        internal IntPtr Exn { get; }

        internal WasmException(string message, List<TrapFrame> frames, Dictionary<nuint, object?> values, IntPtr exn)
            : base(message)
        {
            Frames = frames;
            Values = values;
        ...
        internal static WasmException FromTrapAndException(StoreContext context, IntPtr trap, IntPtr exn)
        {
            ...
            var count = Function.Native.wasmtime_exn_field_count(context.handle, exn);
            for (nuint i = 0; i < count; i++)
            {
                var error = Function.Native.wasmtime_exn_field(context.handle, exn, i, out var value);
                if (error == IntPtr.Zero)
                {
                    var obj = value.ToObject(context.Store);
                    values.Add(i, obj);
                }
                else
                {
            ...
            // todo: We expect `exn` to be passed back into wasmtime, relinquishing ownership.
            //       But if the user catches it and doesn't rethrow where it goes back into wasmtime, we need to handle cleanup via `wasmtime_exn_delete`.
            return new WasmException(accessor.Message, accessor.GetFrames(), values, exn);
        }
    }
}
